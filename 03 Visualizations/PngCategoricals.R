...
#quantitative <- function(df, i) {
#  names(df) <- c("i")
#  ggplot(df, aes(x=i)) + geom_histogram() + labs(title = paste("NUMBER of CONTRIBUTORS by", i)) + labs(x = i, y = "Number of Contributors") + theme(plot.title=element_text(size=20, face="bold", vjust=1, family"Bauhaus 93"))
#}
...
#    q <- data.frame(fromJSON(getURL(URLencode('129.152.144.84:5001/rest/native/?query="select \\\""i"\\\", count(*) n from texas_fed group by \\\""i"\\\" "'),httpheader=c(DB='jdbc:oracle:thin:@129.152.144.84:1521:ORCL', USER='C##cs329e_jc58247', PASS='orcl_jc58247', MODE='native_mode',MODEL='model',returnDimensions = 'False',returnFor = 'JSON', i=i),verbose = TRUE)))
#    o <- quantitative(q,i)
#    print(o) 
#    m[[i]] <- o
#  }
