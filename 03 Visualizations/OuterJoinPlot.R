...
g <- ggplot(MELTEDDF, aes(PARTY, value, color = variable)) + geom_bar(stat="identity") + facet_wrap(~variable) + ggtitle('Averages Fiscal Variables Related to Party') + theme_calc() + labs(x = "Party", y = "Average" ) 
# this second plot is the average contribution based on the party identity of the contributor
j <- ggplot(OUTERJOIN, aes(PARTY, AverageContribution, color = PARTY)) + geom_bar(stat="identity") + ggtitle('Averages Contribution Related to Party') + theme_calc() + labs(x = "Party", y = "Average Contribution" ) 
